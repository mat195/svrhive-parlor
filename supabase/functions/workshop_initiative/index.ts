...
Deno.serve(async (req) => {
  ...
  for (const c of (out.corpus ?? []).slice(0, 3)) {
    ...
    const { data } = await admin.from('action_queue').insert({ kind: 'corpus-initiative', status: 'proposed', payload: { title: `Corpus page: ${c.target_query}`, target_query: c.target_query, rationale: c.rationale, generated_by: 'workshop_initiative', priority: 2 } }).select('id').single();
    if (data?.id) { filed++; ids.push(data.id); }
  }
  // Audit (max 1).
  if (out.audit?.focus) {
    const { data } = await admin.from('action_queue').insert({ kind: 'audit-initiative', status: 'proposed', payload: { title: `Audit: ${out.audit.focus}`, focus: out.audit.focus, rationale: out.audit.rationale, generated_by: 'workshop_initiative', priority: 2 } }).select('id').single();
    if (data?.id) { filed++; ids.push(data.id); }
  }
